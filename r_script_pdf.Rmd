...
| **Samples received within 2 days (GOAL: 95%)** | `r format(h$rec_in_2_days, big.mark=",", trim=TRUE)` | `r round(h$percent_rec_in_2_days, digits=2)`% | `r h$rank_percent_within_goal` of `r state$submitters` | `r round(state$percent_rec_in_2_days, digits=2)`% |
| &nbsp;&nbsp;&nbsp;&nbsp;Met goal? | `r ifelse(h$met_goal==1, 'No', 'Yes')`|  |  |  | 
| **Samples collected at < 24 hours of age** | `r h$col_less_than_24_hours` | `r round(h$percent_less_than_24_hours, digits=2)`% | `r h$rank_early_collection` of `r state$submitters` | `r round(state$percent_less_than_24_hours, digits=2)`%
| **Samples transfused prior to collection** |  `r h$trans` | `r round(h$trans_percent, digits=2)`% |  `r h$rank_transfused` of `r state$submitters`  |  `r round(state$trans_percent, digits=2)`% |
| **Unsatisfactory samples** | `r h$unsat_count` |  `r round(h$unsat_percent, digits=2)`% | `r h$rank_unsats` of `r state$submitters`  | `r round(state$unsat_percent, digits=2)`% |

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;    \* **Note**: Lower numbers indicate higher ranks&nbsp;&nbsp;  
...
if (line_chart == 'quarterly') {
  ...
  p1 <- ggplot(bound, aes(x=QUARTER, y=avg_transit_time, colour=SUBMITTERNAME, group=rev(SUBMITTERNAME))) +
    ...
    geom_text(color="green4", size=3, aes(x=(as.numeric(min(bound$QUARTER)) + .18), 
              y=min_overall_transit, label="Goal: under 2 days")) + 
  ...
  p2 <- ggplot(bound, aes(x=QUARTER, y=unsat_percent, colour=SUBMITTERNAME, 
    ...
    theme(legend.text = element_text(size = 10)) +
    theme(plot.title = element_text(size = 10, face = "bold")) +
    theme(plot.margin = unit(c(0.3,0.3,0.3,0.5), "cm"))
  
} else {
  
  # Plot average transit time over past 12 months
  p1 <- ggplot(bound, aes(x=MONTH, y=avg_transit_time, colour=SUBMITTERNAME, group=rev(SUBMITTERNAME))) +
    ...
    scale_color_manual(values=c("blue","grey59")) +
    geom_hline(color="green4", aes(yintercept=2), size=2) +
    geom_text(color="green4", size=3, aes(x=(as.numeric(min(bound$MONTH)) + .18), y=min_overall_transit, label="Goal: under 2 days")) + 
    theme(legend.title=element_blank()) + 
    theme(legend.position="bottom") +
    theme(legend.text = element_text(size = 10)) +
    theme(plot.title = element_text(size = 10, face = "bold")) +
